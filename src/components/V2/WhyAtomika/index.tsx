  ...
  return (
    <div className="bg-[#0F0A1F] mt-[30px] md:mt-[100px] pb-[30px] md:pb-[141px]">
      ...
      className="relative container pt-[30px] md:pt-[100px]">
       <h2 className="font-gilroy font-semibold text-[#F9FAFB]  drop-shadow-[0_4px_19.9px_rgba(255,255,255,0.27)] text-[32px] md:text-[48px] leading-[1.1] md:leading-[1.2]  mb-[10px] md:mb-5">{t("version2.whyAtomika.title")}<span className=" text-[40px] md:text-[64px]  bg-[linear-gradient(182.09deg,#C38BFA_1.21%,#A755F7_53.87%,#8216EB_123.92%)]  bg-clip-text text-transparent leading-none">{t("version2.features.atomika")}</span>{t("version2.whyAtomika.title2")}</h2>   
        ...
        <div className=" relative px-9 pt-9 pb-8 rounded-[40px] maxw-[585px] w-full overflow-hidden     bg-[linear-gradient(206.98deg,#C38BFA_-27.16%,#A755F7_25.69%,#8216EB_95.99%)]">
         ...
         />
          <h4 className="text-[#F9FAFB] text-[32px] font-gilroy">{t("version2.whyAtomika.atomika.title")}</h4>
          {atomikaItems.map((item, index) => (
            <p className={`text-[#F9FAFB] flex items-center gap-3 font-outfit  ${index!=6 ?"border-b border-[#FFFFFF14]" :null} py-3`} key={index}><img src="/check.svg" alt="" /> {item}</p>
          ))}
        </div>

        <div className=" px-9 pt-9 pb-8 rounded-[40px] max-w-[585px] w-full bg-[#1B162A]">
          <h4 className="text-[#F9FAFB] text-[32px] font-gilroy" >{t("version2.whyAtomika.otherCourses.title")}</h4>
          {otherCoursesItems.map((item, index) => (
            <p className={`text-[#F9FAFB] flex items-center gap-3 font-outfit  ${index!=6 ?"border-b border-[#FFFFFF14]" :null} py-3`} key={index}> <img src={index === 3? "/danger.png": index === 4  ? "/check.svg" : "/x.png" } alt="" /> {item}</p>
          ))}
        </div>
        </div>
        <div className="absolute  bottom-[-230px] right-[-300px]  hidden lg:block  "> <img src="/pen.png" alt="" /></div>
      </motion.div>
    </div>
  );
};
